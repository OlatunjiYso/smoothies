  ...
  <div class="add-smoothie container z-depth-1">
    ...
    <form @submit.prevent="addSmoothie">
      ...
      <div v-for="(ing, index) in ingredients" class="field ingredient" :key="index">
        ...
        <i class="material-icons delete field" @click="deleteIng(ing)"> delete </i>
      </div>
      <div class="field add-ingredient">
        <label for="add-ingredient">Add an ingredient (press tab to add):</label>
        <input type="text" name="add-ingredient" @keydown.tab.prevent="addIng" v-model="another">
      </div>
      <p class="field center-align" v-if="feedback"> {{feedback}}</p>
  ...
  methods: {
    ...
    addIng() {
      if(this.another) {
        ...
        this.feedback = 'Please a an ingredient followed by tab'
      }
    },
    deleteIng(ing){
